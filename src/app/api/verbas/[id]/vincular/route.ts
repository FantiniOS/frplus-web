...
export async function GET(request: Request, { params }: { params: { id: string } }) {
    try {
        ...
        if (!verba) {
            return NextResponse.json({ error: 'Verba n�o encontrada' }, { status: 404 })
        }
        ...
        console.error('Error fetching pedidos dispon�veis:', error)
...
export async function POST(request: Request, { params }: { params: { id: string } }) {
    try {
        ...
        if (!Array.isArray(pedidoIds) || pedidoIds.length === 0) {
            return NextResponse.json({ error: 'pedidoIds � obrigat�rio e deve ser um array' }, { status: 400 })
        }
        ...
        if (verba.status === 'CANCELADA') {
            return NextResponse.json({ error: 'N�o � poss�vel vincular pedidos a uma verba cancelada' }, { status: 400 })
        }
...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
    try {
        ...
        if (!pedidoId) {
            return NextResponse.json({ error: 'pedidoId � obrigat�rio' }, { status: 400 })
        }
