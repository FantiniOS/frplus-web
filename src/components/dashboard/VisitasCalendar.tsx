...
export function VisitasCalendar({ year, month, clientes = [] }: VisitasCalendarProps) {
  ...
  return (
    <div className="rounded-2xl border border-white/[0.08] bg-gradient-to-br from-[#0f1729] to-[#0a0f1a] p-5 shadow-xl shadow-black/30 h-full flex flex-col">
              ...
              {allDays.map((day, i) => {
                ...
                return (
                  <button
                    ...
                    onMouseEnter={() => setHoveredDia(day)}
                  ...
                  <div className="space-y-2 pb-2">
                    {selectedVisitas.map(v => {
                      ...
                      return (
                        <div 
                          key={v.id} 
                          title={`${timeStr} - ${v.titulo}\nCliente: ${clientName || 'N/A'}${v.observacoes ? `\nObs: ${v.observacoes}` : ''}`}
                          className={`relative rounded-lg border border-white/[0.04] p-3 transition-colors ${v.status === 'REALIZADA' ? 'bg-white/[0.01]' : 'bg-white/[0.02] hover:bg-white/[0.04]'}`}
                        >
                             ...
                             <div className="flex items-center gap-1">
                               {v.status !== 'REALIZADA' && (
                                 <button
                                   ...
                                   className="flex-shrink-0 p-2 rounded-lg bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20 transition-all shadow-sm hover:shadow-emerald-500/20"
                               ...
                               <button
                                 ...
                                 className="flex-shrink-0 p-2 rounded-lg bg-blue-500/10 text-blue-400 hover:bg-blue-500/20 transition-all shadow-sm hover:shadow-blue-500/20"
                                 title="Editar Visita"
                               >
                               ...
                               <button
                                 ...
                                 className="flex-shrink-0 p-2 rounded-lg bg-red-500/10 text-red-500 hover:bg-red-500/20 transition-all shadow-sm hover:shadow-red-500/20"
                               ...
                               {phone && (
                                 <button
                                   ...
                                   className="flex-shrink-0 p-2 rounded-lg bg-[#25D366]/10 text-[#25D366] hover:bg-[#25D366]/20 transition-all shadow-sm hover:shadow-[#25D366]/20"
      ...
      {/* Tooltip Flutuante React State */}
      {hoveredDia && visitasByDay.has(hoveredDia) && typeof window !== 'undefined' && createPortal(
        <div 
          className="fixed z-[9999] pointer-events-none flex flex-col gap-2 bg-gray-900 text-white p-3 rounded-lg shadow-xl shadow-black/50 border border-white/10 w-[280px]"
          style={{ 
            top: mousePos.y + 15, 
            left: mousePos.x + 15 
          }}
        >
          ...
          <div className="flex flex-col gap-1.5">
              ...
              return (
                <div key={v.id} className="text-xs leading-relaxed">
                  <div className="flex items-start gap-1.5">
                    <span className="font-bold text-emerald-400 shrink-0">{timeStr}</span>
                    <span className="text-white/90 font-medium line-clamp-1">{v.titulo}</span>
                  </div>
                  {v.cliente && (
                    <div className="text-gray-400 pl-[38px] line-clamp-1 capitalize text-[10px]">
            ...
            {(visitasByDay.get(hoveredDia)?.length || 0) > 4 && (
              <div className="text-[10px] text-gray-500 italic text-center pt-1 border-t border-white/5">
                + {(visitasByDay.get(hoveredDia)?.length || 0) - 4} compromissos...
              </div>
            )}
          </div>
        </div>,
